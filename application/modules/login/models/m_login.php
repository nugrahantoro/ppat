...
//        die;
    ...
    public function checkPassword($username, $password){
        ...
        $sql = 'select userid, username, password, enctype from sys_users where binary(username) = :username and active=1';
    ...
    function createSession($username){
        ...
        //die;
        $vsql = "select 0; 
            insert into sys_session(sessioncode, username) values(:randdata, 'testing');
            update sys_session set sessioncode = last_insert_id() where sessionid = last_insert_id();
            select last_insert_id() as sessionid;";
        $vparams = array('randdata'=>rand(0, 1000));
...
//            values(:sessioncode, :username, :sessionexpired)";
        $vsql = "update sys_session set sessioncode = :sessioncode, username = :username,
            sessionexpired = :sessionexpired where sessionid = :sessionid";
        $vsqlparams = array('sessioncode'=>$sessioncode, 'username'=>$username, 
            'sessionexpired'=>$ses_expired, 'sessionid'=>$sqid);
        //print_r($vsqlparams);
    ...
    public function getSessionData(){
       ...
       // $vsql = "select * from sys_session where sessioncode = cast(:sessionid as char) and sessionexpired > current_timestamp";
	    $vsql = "select a.*,d.rolename, b.fullname  from sys_session a, sys_users b, sys_userroles c, sys_roles d where a.username = b.username and b.userid = c.userid and c.roleid = d.roleid and  a.sessioncode = cast(:sessionid as char) and a.sessionexpired > current_timestamp";
    ...
    public function getUserInfo($username){
        $vsql = "select a.username, a.fullname
            from sys_users a
